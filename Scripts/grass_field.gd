...
var grass_mask_texture: Texture2D
...
func _ready():
	grass_mask_texture = ImageTexture.create_from_image(Image.create(texture_size, texture_size, false, Image.FORMAT_RGBA8))
	sprite.texture = grass_mask_texture

func _process(delta):
	var uniforms = prepare_img_for_shader(grass_mask_texture.get_image(), circle_queue)
	circle_queue = []
	var updated_image = compute_image(uniforms)
	grass_mask_texture = ImageTexture.create_from_image(updated_image)
	material_override.set_shader_parameter("grass_mask", grass_mask_texture)
	sprite.texture = grass_mask_texture
...
func draw_circle(center: Vector2, radius: float):
	circle_queue.push_back([center, radius])
...
func prepare_img_for_shader(current_image: Image, _circle_queue) -> RID:
	if rd == null:
		init_gpu()
	...
	for circle_draw in _circle_queue:
		arr.append_array([circle_draw[0].x, circle_draw[0].y, circle_draw[1]])
	var coord_bytes = arr.to_byte_array()
	...
	coord_uniform.add_id(rd.storage_buffer_create(max(coord_bytes.size(),1), coord_bytes))
